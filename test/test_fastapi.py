...
class FastapiTest(unittest.TestCase):
    ...
    def test_async_rls_query(self):
        response = self.client.get("/async/users", params={"account_id": 1})
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.json(), ["user1"])

    def test_async_rls_query_with_bypass(self):
        response = self.client.get("/async/all_users", params={"account_id": 1})
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.json(), ["user1", "user2"])
        ...
        async with fastapi_sample.sample_database_setup(fastapi_sample.app):
            ...
            async with httpx.AsyncClient(
                ...
                for _ in range(n):
                    tasks.append(
                        make_request(
                            client, "/async/users", {"account_id": 1}, ["user1"]
                        )
                    )
                for _ in range(n):
                    tasks.append(
                        make_request(
                            client, "/async/users", {"account_id": 2}, ["user2"]
                        )
                    )
                for _ in range(n):
                    tasks.append(
                        make_request(
                            client, "/async/all_users", None, ["user1", "user2"]
                        )
                    )
                results = await asyncio.gather(*tasks)
